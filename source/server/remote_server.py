...
the only thing meant to read these:
    ...
    GET /stream?w=960&fps=12&q=60&mon=0  multipart/x-mixed-replace, never ends
    GET /frame?w=960&q=60&mon=0          one still frame
    GET /info                            {"w":...,"h":...,"mime":...}
    GET /monitors                        {"monitors":[{"id","name","w","h"}]}

mon=0 is every screen at once; 1, 2, ... are the individual displays, left
to right, so the phone can page between them.
...
class ScreenBackend:
    ...
    def monitors(self):
        """Screens that can be watched.

        Entry 0 is always every screen at once; the rest are the individual
        displays, left to right, so the phone can page through them.
        Each entry is {"id", "name", "w", "h"}.
        """
        raise NotImplementedError

    def size(self, monitor=0):
...
class GdiScreenBackend(ScreenBackend):
    ...
    def __init__(self):
        ...
        self._MONITORENUMPROC = ctypes.WINFUNCTYPE(
            wintypes.BOOL, H, H, ctypes.POINTER(wintypes.RECT),
            ctypes.c_void_p)
        ...
        user.EnumDisplayMonitors.argtypes = [H, ctypes.c_void_p,
                                             self._MONITORENUMPROC,
                                             ctypes.c_void_p]
        user.GetMonitorInfoW.argtypes = [H, ctypes.POINTER(MONITORINFOEXW)]
    ...
    def _surface(self, screen_dc, width, height):
        ...
        # Each watchable area needs a full-size surface and a scaled one, so
        # the ceiling has to clear a few monitors before it counts as churn.
        if len(self._surfaces) > 12:
            self.close()
    ...
    def _displays(self):
        """Every physical display as (x, y, w, h, label), left to right."""
        ctypes = self._ctypes
        found = []
        ...
        def callback(hmonitor, _hdc, _rect, _data):
            ...
            if self._user32.GetMonitorInfoW(hmonitor, ctypes.byref(info)):
                ...
                found.append((r.left, r.top,
                              max(1, r.right - r.left),
                              max(1, r.bottom - r.top),
                              info.szDevice,
                              bool(info.dwFlags & 1)))   # 1 = primary
            return True

        # The callback must stay referenced for the length of the call.
        proc = self._MONITORENUMPROC(callback)
        self._user32.EnumDisplayMonitors(None, None, proc, None)
        found.sort(key=lambda m: (m[0], m[1]))

        out = []
        for i, (x, y, w, h, device, primary) in enumerate(found, start=1):
            label = "Screen %d" % i
            if primary:
                label += " (main)"
    ...
    def _region(self, monitor):
        """Where to capture from. 0 means the whole virtual desktop."""
        if monitor:
            displays = self._displays()
            if 1 <= monitor <= len(displays):
                x, y, w, h, _label = displays[monitor - 1]
                return x, y, w, h
        return self._virtual_screen()

    def monitors(self):
        vx, vy, vw, vh = self._virtual_screen()
        out = [{"id": 0, "name": "All screens", "w": vw, "h": vh}]
        for i, (x, y, w, h, label) in enumerate(self._displays(), start=1):
            out.append({"id": i, "name": label, "w": w, "h": h})
        # A single-monitor PC has nothing to choose between.
        return out if len(out) > 2 else out[:1]
...
class MssScreenBackend(ScreenBackend):
    ...
    def _region(self, monitor):
        # mss already lists index 0 as every screen at once and 1..N as the
        # individual displays, which is exactly the numbering we expose.
        all_mons = self._grabber().monitors
        if 0 <= monitor < len(all_mons):
            return all_mons[monitor]
        return all_mons[0]

    def monitors(self):
        all_mons = self._grabber().monitors
        out = [{"id": 0, "name": "All screens",
                "w": all_mons[0]["width"], "h": all_mons[0]["height"]}]
        for i, mon in enumerate(all_mons[1:], start=1):
            out.append({"id": i, "name": "Screen %d" % i,
    ...
    def monitors(self):
        with self._lock:
            try:
                ...
                return [{"id": 0, "name": "All screens", "w": w, "h": h}]
